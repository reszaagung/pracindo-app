<!-- src/features/accounting/layout/TransactionEntryLayout.vue -->
<template>
    <div class="flex h-screen bg-[#F8FAFC] font-sans text-slate-700 overflow-hidden relative">

        <!-- HEADER MOBILE (Sudah kebal teks kepanjangan dengan min-w-0, shrink-0, truncate) -->
        <header
            class="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white shadow-sm z-30 flex items-center justify-between px-4 gap-3 border-b border-slate-100">
            <div class="flex items-center gap-3 min-w-0">
                <button @click="toggleSidebar"
                    class="p-2 shrink-0 rounded-xl bg-slate-50 text-slate-600 hover:bg-slate-100 active:bg-slate-200 transition-colors">
                    <i class="pi pi-bars text-xl"></i>
                </button>
                <span class="font-bold text-slate-800 text-base md:text-lg truncate">Input Transaksi</span>
            </div>

            <button @click="kembali"
                class="w-9 h-9 shrink-0 bg-slate-900 rounded-xl flex items-center justify-center shadow-md active:scale-95 transition-transform">
        ...
        <aside
            ...
            <div class="flex flex-col items-center w-full gap-6 lg:gap-8">
                <!-- TOMBOL BACK (Hanya tampil di Desktop) -->
            ...
            <!-- AREA BAWAH HAMBURGER MENU (Tombol Home & Logout) -->
            <div class="mt-auto flex flex-col items-center gap-4 relative mb-4">

                <!-- TOMBOL KEMBALI KE DASHBOARD (HOME) -->
                <div class="group relative flex flex-col items-center">
                    <button @click="keDashboard" type="button"
                        class="w-10 h-10 rounded-xl overflow-hidden cursor-pointer border border-slate-200 hover:border-slate-400 bg-white hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center">
                ...
                <div class="group relative flex flex-col items-center">
                    <button @click="keluar" type="button"
                        class="w-10 h-10 rounded-xl overflow-hidden cursor-pointer border border-rose-100 hover:border-rose-400 bg-white hover:bg-rose-50 transition-all shadow-sm flex items-center justify-center">
                        <i class="pi pi-power-off text-rose-400 group-hover:text-rose-600 transition-colors"></i>
                    </button>
                    <span
                        class="absolute -top-10 bg-rose-600 text-white text-[11px] font-semibold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg">
                        Keluar
                    </span>
                </div>
            </div>
...
// Tombol rumah di menu hamburger memaksa balik ke halaman utama
const keDashboard = () => {
    router.push('/')
}
