<!--
  ...
  ==================================================
  Halaman Jadwal Pengiriman (Distribusi Outbound).
-->
<template>
    <div class="flex flex-col w-full animate-fade-in relative">
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
                ...
                <p class="text-xs md:text-sm text-slate-500 mt-1">Pantau rencana muat dan rute pengiriman ke retail atau
                    kustomer.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="flex flex-col gap-1.5 md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" placeholder="Ketik No. DO atau Nama Tujuan..."
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tanggal
                        Pengiriman</label>
                    <input type="date"
        ...
        <div class="bg-white border border-slate-200 rounded-[24px] p-4 md:p-6 shadow-sm w-full min-h-[400px]">
            ...
            <div class="hidden md:block overflow-x-auto custom-scrollbar">
                <table class="w-full text-left text-sm table-auto min-w-[60rem]">
                    <thead class="text-slate-500 bg-slate-50/50">
                        <tr>
                            <th class="py-3 px-4 font-semibold rounded-tl-xl">No. DO / Tanggal</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="item in jadwalDummy" :key="item.id"
                            class="hover:bg-slate-50/50 transition-colors group">
                            <td class="py-3 px-4 align-top">
                                <div class="font-bold text-slate-800">{{ item.no_do }}</div>
                                <div class="text-xs text-slate-500 mt-0.5">{{ item.tanggal }}</div>
                            </td>
                            <td class="py-3 px-4 align-top">
                                <div class="font-bold text-slate-700">{{ item.tujuan_nama }}</div>
                                <div class="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1">
                                    <i class="pi pi-map-marker text-[10px]"></i> {{ item.tujuan_alamat }}
                                </div>
                            </td>
                            <td class="py-3 px-4 align-top">
                                <div class="font-semibold text-slate-700">{{ item.nopol }}</div>
                                <div class="text-xs text-slate-500 mt-0.5">{{ item.supir }}</div>
                            </td>
            ...
            <div class="md:hidden flex flex-col gap-4">
                <div v-for="item in jadwalDummy" :key="'mob-' + item.id"
                    ...
                    <div class="flex justify-between items-start mb-3 border-b border-slate-100 pb-3">
                        <div>
                            <div class="font-bold text-slate-800 text-sm">{{ item.no_do }}</div>
                    ...
                    <div class="flex flex-col gap-2 mb-3">
                        <div>
                            <span
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">Tujuan</span>
                            <div class="font-semibold text-slate-700 text-sm">{{ item.tujuan_nama }}</div>
                            <div class="text-[11px] text-slate-500 line-clamp-1">{{ item.tujuan_alamat }}</div>
                        </div>
                        <div>
                            <span
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">Armada</span>
                            <div class="text-sm font-medium text-slate-700">{{ item.nopol }} &bull; {{ item.supir }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- State Kosong -->
            <div v-if="jadwalDummy.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
                <div
...
// Data Dummy Sementara (sebelum dihubungkan dengan API Backend)
const jadwalDummy = ref([
    {
        id: 1,
        no_do: 'DO-202608-001',
        tanggal: '07 Agustus 2026',
        tujuan_nama: 'PT. Mitra Bangunan Sejahtera (Retail)',
        tujuan_alamat: 'Jl. Raya Industri Km 14, Bekasi',
        nopol: 'B 9182 TXY',
        supir: 'Budi Santoso',
        status: 'TERJADWAL'
    },
    ...
    },
    {
        ...
        tujuan_nama: 'CV. Karya Abadi',
        tujuan_alamat: 'Jl. Gatot Subroto, Bandung',
        nopol: 'D 1234 CD',
        supir: 'Jajang',
        status: 'DIKIRIM'
    }
