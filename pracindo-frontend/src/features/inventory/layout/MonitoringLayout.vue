<!--
  features/inventory/layout/MonitorLayout.vue
  ===============================================
  Wrapper Layout untuk Monitor Tangki, Stok, dan Distribusi Klaim.
  Menggunakan standar desain Mini-Sidebar Enterprise.
-->
<template>
    <div class="flex h-screen bg-[#F8FAFC] font-sans text-slate-700 overflow-hidden relative">
        ...
        <header
            ...
            <div class="flex items-center gap-3">
                ...
                <span @click="kembali"
                    class="font-bold text-slate-800 text-base md:text-lg cursor-pointer hover:text-slate-500 transition-colors">
                    Inventory Monitor
                </span>
            </div>
        ...
        <div v-if="sidebarAktif" @click="tutupDiMobile"
            class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-40 lg:hidden transition-opacity"></div>

        <!-- Sidebar Kiri Dinamis -->
        <aside
            class="bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 flex flex-col items-center py-6 flex-shrink-0 justify-between transition-transform duration-300 ease-in-out"
            :class="[
