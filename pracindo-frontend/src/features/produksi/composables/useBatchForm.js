...
export function useBatchForm() {
    ...
    // Jenis mutlak ditentukan dari ada tidaknya sumber WIP.
    const jenis = computed(() =>
        form.baris.some((b) => b.sumber === 'WIP' && b.batch_sumber) ? 'BLENDING' : 'MIXING',
    )

    /** Baris yang siap dikirim. Baris setengah terisi diabaikan diam-diam. */
    function payload() {
        const input_raw = []
        const input_wip = []
    ...
    async function hitung() {
        ...
        try {
            ...
            // BUANG RESPONS BASI: Pastikan hanya memproses respons dari request terakhir
            if (ini === seq) pratinjau.value = hasil
        } catch (e) {
            // Kalkulator pratinjau idealnya selalu 200, tapi fallback jika jaringan putus
            if (ini === seq) {
    ...
    // Panggil pratinjau setiap kali ada perubahan data (ter-debounce)
    watch(
    ...
    // --- Galat per baris, untuk highlight field yang bermasalah ---
    ...
    /**
     * Simpan DRAFT lalu langsung POSTING.
     * Kalau posting gagal, draftnya TETAP ADA agar isian operator tidak hilang
     * hanya karena satu baris kehabisan saldo sesaat sebelum disubmit.
     */
    async function simpanDanPosting() {
        ...
        try {
            ...
            // Beri tahu global store bahwa mutasi terjadi, periksa invariant!
            ...
            galatServer.value = e
            if (draft) e.draftId = draft.id // Tempelkan ID draft ke error untuk notifikasi
            throw e
        } finally {
