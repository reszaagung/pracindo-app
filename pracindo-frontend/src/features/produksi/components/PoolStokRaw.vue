<!-- src/features/produksi/components/PoolStokRaw.vue -->
<template>
        ...
        <form @submit.prevent="prosesPemakaian" class="flex flex-col gap-4">
            <div>
                ...
                <select v-model="form.produk_id" required 
                    ...
                    <option v-for="item in opsiPool" :key="item.id || item.produk_id" :value="item.id || item.produk_id">
                        {{ item.produk_kode || item.kode }} ({{ item.produk_nama || item.nama }})
                    </option>
                </select>
            </div>
...
onMounted(async () => {
    try {
        const { data } = await api.get('v1/produksi/pool/')
        opsiPool.value = data.rincian || data.results || data || []
    } catch (err) {
        galatMutasi.value = 'Gagal memuat daftar pool raw.'
    }
})
...
const prosesPemakaian = async () => {
    ...
    try {
        await api.post('v1/produksi/mutasi/tarik/', form.value)
        suksesMutasi.value = true
        form.value.qty = null
        form.value.referensi = ''
        emit('pemakaian-sukses')
    } catch (err) {
        galatMutasi.value = err.response?.data?.detail || 'Gagal mencatat pemakaian stok.'
    } finally {
