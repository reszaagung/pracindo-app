      ...
      <table class="w-full text-sm text-left whitespace-nowrap">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr class="bg-slate-50 font-semibold text-slate-700 border-t-2 border-slate-200">
            <td colspan="2" class="px-4 py-3 text-right uppercase text-xs tracking-wider text-slate-500">Total Input</td>
            <td class="px-4 py-3 text-right font-mono">{{ formatKg(hasil?.total_qty_masuk) }}</td>
            <td class="px-4 py-3 text-right text-slate-400">-</td>
            <td class="px-4 py-3 text-right text-slate-800">{{ formatRupiah(hasil?.total_nilai_masuk) }}</td>
            <td></td>
          </tr>
          
          <!-- Baris Hasil Valuasi (Yield) -->
          <tr class="bg-emerald-50 font-bold text-emerald-800 border-t border-emerald-100">
            <td colspan="2" class="px-4 py-3.5 flex items-center justify-end gap-2 uppercase text-xs tracking-wider">
            ...
            <td class="px-4 py-3.5 text-right text-emerald-700">{{ formatRupiah(hasil?.wip_cost_per_kg) }}</td>
            <td class="px-4 py-3.5 text-right text-emerald-700">{{ formatRupiah(hasil?.total_nilai_masuk) }}</td>
            <td></td>
          </tr>
        </tbody>
      </table>
