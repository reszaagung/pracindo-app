...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'pracindo.settings') # Sesuaikan nama settings jika berbeda
django.setup()
...
print("Memulai proses memasukkan data Master Produk...")
dibuat = 0
diperbarui = 0

for item_id, nama in items.items():
    if not item_id.strip():
        continue
    obj, created = MasterProduk.objects.update_or_create(
        id=item_id.strip(),
        defaults={'nama_item': nama.strip()}
    )
    if created:
        dibuat += 1
    else:
...
print("--- SELESAI ---")
print(f"Data baru ditambahkan: {dibuat}")
print(f"Data diperbarui: {diperbarui}")